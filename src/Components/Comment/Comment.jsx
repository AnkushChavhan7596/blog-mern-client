  ...
  const handleCommentDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(
        url + `/api/comments/delete-comment/${id}`
      );
    ...
    const fetchCommentAuthor = async () => {
      try {
        const res = await axios.get(
          url + `/api/auth/user/${comment?.authorId}`
        );
        ...
        <div className="header_name_comment">
          <div className="h1_cl">
            <p>
              ...
              {post?.authorId === comment.authorId ? (
                <span className="author_txt_txt">Author</span>
              ) : (
                ""
              )}
            </p>
          ...
          <div className="h1_cr">
            {activeUser?._id === comment.authorId ? (
              <p
                className="comment_del_com"
                onClick={() => handleCommentDelete(comment._id)}
              >
                <DeleteIcon fontSize="small" className="comment_del" />
              </p>
            ) : (
              ""
            )}
