...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchActiveUser = async () => {
      try {
        if (Cookies.get("jwt")) {
          const res = await axios.post(url + "/api/auth/active-user", {
    ...
    const fetchUsers = async () => {
      try {
        const res = await axios.get(url + "/api/auth/all-users");
    ...
    const fetchComments = async () => {
      try {
        const res = await axios.get(url + "/api/comments/all-comments");
      ...
      <div className="home_wrapper">
        <div className="leftHome">
          {Cookies.get("jwt") && activeUser ? (
            allPosts?.length === 0 ? (
              <p className="posts_not_found">No Posts of {cate} Category 😢</p>
            ) : (
              allPosts?.map((post) => {
        ...
        {activeUser ? (
          <div className="rightHome">
            ...
            <div className="categories_wrapper">
              ...
              <span
                className="category_item"
                onClick={() => filterCategory("pollution")}
              >
                Pollution
              </span>
              <span
                className="category_item"
                onClick={() => filterCategory("covid")}
              >
                Covid
              </span>
              <span
                className="category_item"
                onClick={() => filterCategory("war")}
              >
                War
              </span>
